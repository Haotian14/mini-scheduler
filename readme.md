...
这是一个面向本地开发与小型集群的 **轻量级分布式任务调度系统（Nimbus Scheduler）**。
...
Dashboard 提供生产级的集群概览、任务创建与筛选、连接状态提示、失败自动重连、历史与实时日志查看，并适配桌面和移动端。
...
- 可直接创建任务，并按状态或关键词筛选
- 展示成功率、平均耗时、在线率等关键指标
- WebSocket 断开后自动重连，恢复当前日志订阅

### 6. 可靠性与可运维性

- Master 和 Worker 均提供 `GET /health` 健康检查
- Master 对任务参数进行严格校验，并支持跨域调用
- Worker 离线时，其运行中的任务会自动返回待调度队列
- 分发失败会正确释放预占资源，避免资源泄漏
